...
function ERACombatDOTracker:Create(timers, enemiesTracker, spec, fillerDPSFunction)
    ...
    dt.fillerDPSFunction = fillerDPSFunction
    dt.allDOTs = {}
    dt.activeDOTsByID = {}
    dt.targetDOTs = {}
...
function ERACombatDOTracker:updateAsNested_returnHeightForTimerOverlay(t)
    ...
    local fillerDPS = self.fillerDPSFunction(self)
    ...
    for i, d in ipairs(self.targetDOTs) do
        y = d:draw(y, fillerDPS, hasteMod) + ERACombat_TimerBarSpacing
    end
    return y
end
...
function ERACombatDOTDefinition:create(
    ...
    def.talent = talent
    def.instances = {}
    def.remDurationOnCurrentTarget = 0
    def.totDurationOnCurrentTarget = 1
...
function ERACombatDOTDefinition:computeDamageDOT(offset, fillerDPS, duration, hasteMod)
    if (self.overridefillerDPS) then
        fillerDPS = self.overridefillerDPS()
    end
    local lifeExpectancy = self.currentTargetInfo.lifeExpectancy - offset
    if (lifeExpectancy > 0) then
        local instantDamage, overTimeDamage, damageModifidedByHaste, spreadsToNearbyTargets, instantDealtToNearbyTargets = self.damageFunction(self, self.currentTargetInfo)
        local dps = overTimeDamage / duration
        if (damageModifidedByHaste) then
            dps = dps / hasteMod
        end
        ...
        return dmg, fillerDPS
    else
        return 0, fillerDPS
    end
end
function ERACombatDOTDefinition:draw(y, fillerDPS, hasteMod)
    ...
    local castTimeForDPS
    if (self.castTime > 0) then
        castTimeForDPS = self.castTime
    else
        castTimeForDPS = 1.5
    end
    ...
    if (self.limitedInstances) then
        ...
        if (minTar) then
            if (minTar == self.currentTargetInfo) then
                ...
                if (inPandemicWindow) then
                    local dmg
                    dmg, fillerDPS = self:computeDamageDOT(castTime, fillerDPS, duration, hasteMod)
                    self.isWorthRefresing = fillerDPS < (dmg / castTimeForDPS)
                else
        ...
        if (self.currentTargetInfo) then
            ...
            local dmg
            dmg, fillerDPS = self:computeDamageDOT(offset, fillerDPS, duration, hasteMod)
            if ((dmg / castTimeForDPS) > fillerDPS) then
                self.isWorthRefresing = inPandemicWindow
                self.willBeWorthRefreshing = true
            else
                self.isWorthRefresing = false
