...
function ERACombatGrid:updateGroup()
    ...
    if (self.isGridVisible) then
        self.hasTank = false
        self.hasHealer = false
        self.hasDPS = false
        for _, u in pairs(self.unitsByID) do
            u:updateMemberStatus()
            if (u.role == "TANK") then
                self.hasTank = true
            elseif (u.role == "HEALER") then
                self.hasHealer = true
            else
                self.hasDPS = true
            end
...
function ERACombatGrid:UpdateIdle(t)
    if (t - self.lastInspects > 2) then
        self.lastInspects = t
        --[[
            on verra plus tard pour utiliser NotifyInspect(+INSPECT_READY(GUID)) et GetSpecializationRoleByID
        for id, u in pairs(self.unitsByID) do
            NotifyInspect(id)
        end
        ]]
    end
